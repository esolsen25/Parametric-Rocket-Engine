...
if __name__ == "__main__":
    # Finding and importing MATLAB calculated data
    dir = os.getcwd()+"\\temp"
    filename = pjoin(dir,"to_python.mat")
    mat = spio.loadmat(filename,squeeze_me=True)
    # Constants
    k = mat['k']  # [-] ratio of specific heats at the throat
    l_percent = 80  # nozzle length percntage (60, 80, 90)

    # typical upper stage values
    aratio = mat['AeAt'] # [-] Ae/At
    throat_radius = round(mat['R_throat_imp'],4) # [cm] Throat Radius
    
    # rao_bell_nozzle_contour
    angles, contour = bell_nozzle(k, aratio, throat_radius, l_percent)
    filename = pjoin(dir,"engine_contour.mat")
    spio.savemat(filename,{"engine_contour":contour})
    # plot contour
